...
class EnvironmentRect:
    ...
    def __init__(self, d, radius=None):
        if radius is not None:
            ...
            pos = np.array([d.player.position.future.x,
...
def get_environment(d, radius=None):
    env_rect = EnvironmentRect(d, radius)
    env = np.zeros((env_rect.drect.height, env_rect.drect.width),
                   dtype=np.uint16)
    ...
    for j in range(env_rect.srect.height):
        for i in range(env_rect.srect.width):
            ...
            if spos == (d.player.position.future.x,
                        d.player.position.future.y):
                # Player
            ...
            dpos = (env_rect.drect.lt[1] + j, env_rect.drect.lt[0] + i)
            env[dpos] = s
...
class DiabloGame:
    ...
    def update_ticks(self):
        missed = self.state.game_tick - self.last_tick
        self.last_tick += missed
        return missed

    def same_ticks(self):
        diff = self.state.game_tick - self.last_tick
        return diff == 0

    def submit_key(self, key):
        # Busy-loop for next tick. Important to understand, that every
        # key has press and release phases, so we can't submit same
        # two keys sequentially if one was not released, otherwise the
        # second key will be lost.  With the following loop we
        # introduce 1 tick delay from the previous key acceptance.
        while self.state.game_tick < self.acceptance_tick + 1:
            time.sleep(0.01)
        ...
        self.acceptance_tick = self.state.game_tick
