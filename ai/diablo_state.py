...
class DungeonRect:
    ...
    def __init__(self, d=None, radius=None, dim=None):
        assert (radius is None) ^ (dim is None)
        ...
        if radius:
            ...
            self.lt = np.array([x_min, y_min])
            self.rb = np.array([x_max, y_max])

            self.width  = self.rb[0] - self.lt[0]
            self.height = self.rb[1] - self.lt[1]
        else:
            self.lt = np.array([0, 0])
            self.rb = np.array([dim[0], dim[1]])

            self.width  = self.rb[0]
            self.height = self.rb[1]
...
def get_environment(d, dun_rect=None):
    if not dun_rect:
        # Whole dungeon
        dun_rect = DungeonRect(dim=(d.maxdun))
    env = np.zeros((dun_rect.height, dun_rect.width), dtype=np.uint16)

    for i_off in range(dun_rect.width):
        i = dun_rect.lt[0] + i_off

        for j_off in range(dun_rect.height):
            j = dun_rect.lt[1] + j_off
            pos = (i, j)
            obj = to_object(d, pos)
                ...
                if obj is not None:
                    if is_barrel(obj) and obj._oSolidFlag:
                        ...
                        if obj.selectionRegion != 0:
                            s |= EnvironmentFlag.ChestClosed.value
                        else:
                            s |= EnvironmentFlag.ChestOpened.value
                    elif is_sarcophagus(obj):
                        if obj.selectionRegion != 0:
                            s |= EnvironmentFlag.SarcophClosed.value
                        else:
                            s |= EnvironmentFlag.SarcophOpened.value
                if d.dItem_np[pos] > 0:
            ...
            if pos == (d.player.position.future.x,
                ...
                if d.player._pmode == PLR_MODE.PM_DEATH.value:
                    s |= EnvironmentFlag.PlayerDead.value
                else:
...
def get_surroundings(d, dun_rect):
    env = get_environment(d, dun_rect)
    ...
    for i, row in enumerate(env):
        for j, tile in enumerate(row):
            ...
            if tile & EnvironmentFlag.Barrel.value:
                s = 'B'
            if tile & EnvironmentFlag.ChestClosed.value:
                s = 'C'
            if tile & EnvironmentFlag.ChestOpened.value:
                s = 'c'
            if tile & EnvironmentFlag.SarcophClosed.value:
                s = 'S'
            if tile & EnvironmentFlag.SarcophOpened.value:
                s = 's'
            ...
            if tile & EnvironmentFlag.Player.value:
                s = 'o'
            if tile & EnvironmentFlag.PlayerDead.value:
                s = 'X'
            surroundings[i, j] = s
...
class DiabloGame:
    def __init__(self, state_dir, proc, mmapped, state):
    ...
    def run(config):
        ...
        state_dir = tempfile.TemporaryDirectory(prefix="diablo-")
        ...
        mshared_path = os.path.abspath(state_dir.name + "/shared.mem")
        mmapped, state = DiabloGame.attach_mshared_path(mshared_path)
        return DiabloGame(state_dir, proc, mmapped, state)

    @staticmethod
    def attach(mshared_path):
        mmapped, state = DiabloGame.attach_mshared_path(mshared_path)
        return DiabloGame(None, None, mmapped, state)
