...
def get_events_as_string(game, events):
    ...
    return " ." * (events.queue.maxlen - cnt) + s
...
def display_diablo_state(game, stdscr, events, envlog, missed_ticks):
    ...
    events_str = get_events_as_string(game, events)

    msg = "Total: mons HP %d, items %d, objs %d, lvl %d | %s" % \
        (total_hp, items_cnt, obj_cnt, d.player.plrlevel, events_str)
    msg = truncate_line(msg, width - 1)
...
#            minibatch_size=512,
    ...
    if args['train-ai']:
        if args['--save-to-checkpoint']:
            ...
            with open(CHECKPOINT_PATH + "/cmdline.txt", "w") as f:
            ...
            # Dump gameconfig
            with open(CHECKPOINT_PATH + "/gameconfig.json", "w") as f:
                json.dump(convert_paths(gameconfig), f, indent=4)
