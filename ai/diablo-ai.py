...
Usage:
  diablo-ai.py play     [--attach=MEM_PATH_OR_PID] [--no-monsters] [--no-env-log]
  diablo-ai.py play-ai  [--attach=MEM_PATH_OR_PID] [--no-monsters]
  diablo-ai.py train-ai [--attach=MEM_PATH_OR_PID] [--no-monsters] [--seed=SEED] [--same-seed] [--train-batch-size=SIZE] [--train-iters=ITERS] [--gpus=NR] [--env-runners=NR] [--tune] [--log-to-stdout] [--no-actions] [--restore-from-checkpoint] [--save-to-checkpoint]
  diablo-ai.py list
  diablo-ai.py (-h | --help)
...
Options:
  ...
  --no-monsters              Disable all monsters on the level
  --no-env-log               No environment log is shown on the TUI screen, which can speed up refresh rate on slow terminals (default: enabled, meaning that if a log is produced by the Gymnasium environment, it will be displayed)
  --seed=SEED                Initial seed (default: 0)
  --same-seed                Set same seed for all runners (default: seed += worker_index - 1)
  ...
  --log-to-stdout            Write logs to stdout (default: env.log file in Diablo state folder)
...
def display_matrix(dunwin, m):
    ...
    for row in range(rows):
        for col in range(cols):
            _addch(dunwin, row + y_off, col + x_off, m[row, col])
