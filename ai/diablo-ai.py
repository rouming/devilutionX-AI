...
def display_diablo_state(stdscr, d_shared, missed_ticks):
    ...
    def interesting_objects(oid):
        obj = d.Objects[oid]
        if diablo_state.is_door(obj):
            return 1 if diablo_state.is_door_closed(obj) else 0
        elif diablo_state.is_barrel(obj):
            return 1 if obj._oSolidFlag else 0
        elif diablo_state.is_chest(obj) or diablo_state.is_sarcophagus(obj):
    ...
    dun_rect = diablo_state.DunRect(d=d, radius=radius)
