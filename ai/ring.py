...
class RingQueue(ctypes.Structure):
    ...
    def has_capacity_to_submit(self):
        """Returns true if enough capacity to submit a new entry."""
        return self.write_idx - self.read_idx < RING_QUEUE_CAPACITY
    ...
    def get_entry_to_retrieve(self, read_idx=None):
        ...
        if write_idx == read_idx:
            return None
        return self.array[read_idx & RING_QUEUE_MASK]
...
if __name__ == "__main__":
    ...
    ring = RingQueue(ctypes.addressof(ctypes.c_char.from_buffer(shm)))
    ...
    res = ring.has_capacity_to_submit();
    assert(res)

    # Consume the whole capacity
    i = 0
    while ring.has_capacity_to_submit():
