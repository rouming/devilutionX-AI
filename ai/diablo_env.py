...
class DiabloEnv(gym.Env):
    ...
    def pause_game(self, pause=True):
        if self.paused != pause:
            ...
            if pause:
                ...
                # Advance action time by the duration we were paused
                self.last_action_time += time.time() - self.pause_time

            if pause ^ game_paused:
                print("PAUSING" if pause else "CONTINUING", file=self.log)
    ...
    def is_agent_stuck(self, d, was_exploring):
        ...
        if was_exploring or np.any(np.abs(self.last_player_pos - p) > 10):
            self.last_action_time = t
            self.last_player_pos = p
            return False

        # Check if agent was not exploring for some time
        return t - self.last_action_time >= 60
    ...
    def evaluate_step(self, d, env, action):
        ...
        was_exploring = reward != NOP_PENALTY
