...
def lookup_dependent_types(type_infos_dict):
    ...
    # Remove primitive types, like integers or pointers
    sorted_type_names = [n for n in sorted_type_names \
                         if (ti := type_infos_dict[n]) and \
                         ti['class'] not in ('integer', 'pointer')]
...
def generate_type(type_info, type_infos_dict):
    ...
    def to_ctypes(type_info, type_infos_dict):
        ...
        if blacklisted(type_info):
            ...
            # This handles all pointers through the 'void *'.
            # TODO: make smarter
            ...
            typename = ctypes_primitives[typename]
...
def generate_types_and_variables(sorted_type_names, type_infos_dict, variables):
    ...
    vars_list = [{**v, 'type': Unquoted(strip_namespaces(v['type_name']))} \
                 for v in variables]
