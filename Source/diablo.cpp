...
static void update_shared_state(void)
{
	...
	// On early start, the game state is not completely initialized, so
	// incoming keys may not work. Delay keys processing. The number of
	// ticks to delay was chosen empirically.
	...
	// Increase tick after states are updated and keys injected
	shared::game_tick++;
}
